...
class LifeHackSerializerTestCase(APITestCase):
    ...
    def test_serializer_missing_title(self):
        data = {"description": "This is", "tag_ids": [self.tag1.id, self.tag2.id]}
        serializer = LifeHackSerializer(data=data)
...
class UserProfileSerializerTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.profile_a = UserProfile.objects.create(
            user=self.user_a, bio="Hello, I’m A"
        )
    ...
    def test_user_profile_serialization(self):
        ...
        self.assertEqual(
            serializer.data["following_set"][0]["user"]["username"], "user_b"
        )
...
class CommentSerializerTest(APITestCase):
    def setUp(self):
        ...
        self.comment = Comment.objects.create(
            user=self.user, hack=self.lifehack, content="This is a test comment."
        )
